...
def generate_random_MIP(num_vars, num_constraints):
    ...
    c = np.random.randint(0, 50, num_vars)

    # generate random coefficients for the constraints
    A = np.random.randint(0, 25, size=(num_constraints, num_vars))
    b = np.random.randint(0, 40, num_constraints)
...
def generate_dataset(no_problems):
    ...
    for i in range(no_problems):
        num_vars = random.randint(10, 3000)
        num_constraints = random.randint(10, 3000)
